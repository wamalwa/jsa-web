                    ...
                    <div class="row" v-else>
                        ...
                        <div class="col-md-6 col-lg-4" v-for="article in page">
                            <div class="blog-item border mb-5">
                                <router-link class="blog-post-image" :to="`/blogdetail/${article.id}/${article.slug}`">
                                      <img :src="article.image_url" alt="blog-img" class="img-fluid w-100">
                                </router-link>
                                <div class="blog-post-info">
                                    <h4 class="blog-post-title">
                                        <router-link class="text-dark" :to="`/blogdetail/${article.id}/${article.slug}`">
                                            {{ article.title }}
                                        </router-link>
                                    </h4>
                                    <p>{{ article.main_tag_line }}</p>
                                    <div class="post-event d-flex align-items-center">
                                        <a href="#" class="blog-info-meta text-dark">{{ article.date_published | formatDate }}</a>
                                        <a href="#" class="blog-category text-dark">{{ article.category_name }}</a>
                                    </div>
                                    <router-link class="blog-read-more" :to="`/blogdetail/${article.id}/${article.slug}`">
    ...
    export default {
        ...
        mounted() {
            if(this.$route.params.categoryId != null) {
                this.defaultTitle = `News in ${this.$route.params.categoryName} Category`;
                this.$store.dispatch("news/GET_CATEGORY_NEWS_COUNT", this.$route.params.categoryId);
                this.$store.dispatch("news/GET_CATEGORY_PAGE", {
